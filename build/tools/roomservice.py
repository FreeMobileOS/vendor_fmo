...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from FreeMobileOS Github (http://github.com/FreeMobileOS)." % device
...
page = 1
while not depsonly:
    request = Request("https://api.github.com/users/FreeMobileOS/repos?page=%d" % page)
...
def is_in_manifest(projectname, branch):
    try:
        lm = ElementTree.parse(".repo/local_manifests/fmo_device.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'FreeMobileOS/%s already exists' % (repo_name)
            else:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": repo_name, "revision": fmo_branch })
    ...
    f = open('.repo/local_manifests/fmo_device.xml', 'w')
    f.write(raw_xml)
    f.close()
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for FreeMobileOS/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: FreeMobileOS/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "FreeMobileOS/%s" % repo_name, "revision": fmo_branch })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/fmo.dependencies'
    syncable_repos = []
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("device_") and repo_name.endswith("_" + device):
            ...
            print "HACK:Found repository: Manufacture:%s" % (manufacturer)
            repo_path = "device/%s/%s" % (manufacturer, device)

            add_to_manifest([{'repository':repo_name,'target_path':repo_path,'branch':fmo_branch}])
...
print "Repository for %s not found in the FreeMobileOS Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/fmo_device.xml" % device
